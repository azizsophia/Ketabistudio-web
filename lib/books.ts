...
export type Look = "light" | "medium" | "dark";

/** Web look → PSD character-variant group (modesty_pipeline.set_variant). */
export const LOOK_TO_PSD: Record<Look, string> = {
  light: "Blonde light",
  medium: "Blonde dark",
  dark: "Dark",
};
...
export type Book = {
  ...
  personalization:
    | {
        ...
        fields: { name: { min: 1; max: 14 }; look: Look[] };
        appearsOn: string[];
      }
    | {
        type: "fixed";
...
export const BOOKS: Book[] = [
  {
    ...
    personalization: {
      type: "personalized",
      pipeline: "modesty_pipeline",
      fields: { name: { min: 1, max: 14 }, look: ["light", "medium", "dark"] },
